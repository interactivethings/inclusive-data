...
export class Histogram extends React.Component {
	...
	updateDataMelody = (bins, maxBin, focus) => {
		...
		const dataMelody = bins.map((bin, i) => {
			return {
				time: i + 1,
				note: Tone.Frequency('A1').transpose(toneScale(bin.length)).toFrequency(),
				velocity: 0.5
			};
		});
		return dataMelody;
	...
	triggerDataMelody = () => {
		...
		const indicatorScale = scaleLinear().domain([ minValue, maxValue ]).range([ 0, CHART_WIDTH ]);
	...
	moveFocusToNextDataPoint = (e) => {
		...
		if (e.key === 'ArrowRight') {
			this.setState(
				{
					focusedBar: this.state.focusedBar === binsNb - 1 ? 0 : this.state.focusedBar + 1
				},
	...
	render() {
		...
		const maxBin = max(bins, (d) => d.length);
		const binScale = scaleLinear().domain([ minBin, maxBin ]).range([ CHART_HEIGHT, 0 ]).nice();
	...
	render() {
		...
		return (
			<div className="histogram-controls">
				...
				<label htmlFor="histogram-play-data-melody" aria-describedby="histogram-play-button-description">
...
class Chart extends React.Component {
	...
	componentDidUpdate(prevProps) {
		...
		if (prevProps.focusedBar !== this.props.focusedBar) {
			this.dataGroup.current.querySelector(`#histogram-bar-${this.props.focusedBar}`).focus();
		}
	}
	...
	render() {
		const { bins, indicatorScale, binScale, binsNb, planetsNb, valueExtent, indicator, indicators } = this.props;
									...
									<rect
										...
										onKeyDown={(e) =>
											e.key === 'ArrowLeft' || e.key === 'ArrowRight'
												...
												: e.key === 'Escape' || e.key === 'Tab' ? this.focusDataGroup : null}
				...
				{bins.map((bin, i) => {
					return (
						<div
							...
							style={{
								transform: `translate(${MARGIN.LEFT + indicatorScale(bin.x0)}px, ${MARGIN.TOP +
									binScale(bin.length)}px)`,
								display: this.props.showTooltip && i === this.props.focusedBar ? 'block' : 'none'
							}}
						>
